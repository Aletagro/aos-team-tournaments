...
const Teams = () => {
    ...
    const sortTeams = useCallback((array) => meta.round || meta.isRostersShow
        ? reverse(sortBy(array, ['win', 'draw', 'tp', 'tp_without_cap']))
        : array
    , [])
    ...
    useEffect(() => {
        fetch('https://aoscom.online/teams/')
    ...
    const handleClickTeam = (_team) => () => {
        if (!player.team_id && !player.isJudge) {
    ...
    const handleSendMessageToCaptain = useCallback(async (captainId) => {
        const message = `${player.surname} ${player.name} вступил в вашу команду`
    ...
    const handleChangeTeam = useCallback(async (team) => {
        await fetch(`https://aoscom.online/players/something/?id=${player?.id}&column=team_id&value=${team.id}`, {
            method: 'PUT',
            headers: {
    ...
    const renderTeam = (team, index) => 
        <button key={index} id={Styles.playerContainer} onClick={handleClickTeam(team)}>
            {renderRow(index + 1, team.name, team.win, team.draw, team.tp, team.tp_without_cap, index % 2)}
        </button>
